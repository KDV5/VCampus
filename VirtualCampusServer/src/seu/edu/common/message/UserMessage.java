...
public class UserMessage extends BasicMessage {
	...
	private int matchOrNot = -1;		// 不存在该用户返回0，密码错误返回1，成功返回2，身份不对返回3
	private int regSucc = -1;			// 注册成功返回1，已有该用户名返回0
	...
	public User setUser(){
		User user = new User(this.userID, this.password ,this.identity);
		return user;
	}
