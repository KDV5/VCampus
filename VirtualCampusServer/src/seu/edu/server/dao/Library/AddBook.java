...
public class AddBook {
	...
	//图书入库
	LibraryMessage addBook(LibraryMessage libMessage){
		...
		try{
		    ...
		    if(rs.next()){
			    ...
			    pst.setInt(1, rs.getInt("Storage")+libMessage.getStorage());  
			    pst.setInt(2, rs.getInt("TotalNumber")+libMessage.getTotalNumber());  
		    	...
		    	//若入库图书未存在
			     sql="insert into tblBooks(BookID,BookName,Author,Place,Storage,TotalNumber,Introduction,Type) values(?,?,?,?,?,?,?)";
			     pst = db.conn.prepareStatement(sql);
			     pst.setString(1,libMessage.getBookID());
			     pst.setString(2,libMessage.getBookName());
			     pst.setString(3,libMessage.getAuther());
